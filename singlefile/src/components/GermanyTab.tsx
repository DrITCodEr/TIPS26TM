...
export function GermanyTab() {
  ...
  const dtIdx = TEAMS.findIndex((team) => team.name === DFB);
    ...
    <section>
      ...
      >
        ...
        <h2 style={{ fontSize: 20, fontWeight: 800, lineHeight: 1.1, letterSpacing: "-0.02em" }}>
          {t.germany.title}<br /><span style={{ color: "var(--mint)" }}>{t.germany.titleGroup}</span>
        </h2>
      ...
      <SectionTitle>{t.germany.matchesTitle}</SectionTitle>
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {dtMatches.map(({ m, i }) => {
          const isHome = m.teamA === DFB;
          const opp = isHome ? m.teamB : m.teamA;
          const oppT = TEAMS.find((team) => team.name === opp)!;
          ...
          return (
            <Card key={i}>
              <div style={{ fontSize: 10, color: "var(--text-tertiary)", marginBottom: 8 }}>
                {formatScheduleDate(m.date, locale)} · {m.time} · {m.venue}
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ fontSize: 20 }}>🇩🇪</span>
                  <span style={{ fontSize: 13, fontWeight: 700 }}>{teamName("Deutschland")}</span>
                </div>
                <span style={{ fontSize: 11, fontWeight: 700, color: "var(--text-tertiary)" }}>vs</span>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ fontSize: 13, fontWeight: 700 }}>{teamName(oppT.name)}</span>
              ...
              {result ? (
                <>
                  ...
                  <div className="match-footer">
                    <span>⌀ {t.germany.avgGoals}: <strong style={{ color: "var(--text-primary)" }}>{avgDFB} : {avgOpp}</strong></span>
                    <span>{t.germany.topScore}: <strong style={{ color: "var(--mint)" }}>{topScore}</strong></span>
                  </div>
                </>
              ) : (
                <div style={{ fontSize: 11, fontStyle: "italic", color: "var(--text-tertiary)" }}>
                  {t.germany.promptForSim}
                </div>
              )}
            </Card>
          );
      ...
      <SectionTitle>{t.germany.koPathsTitle}</SectionTitle>
      <Card style={{ marginBottom: 8 }}>
        <h3 style={{ fontSize: 13, fontWeight: 700, color: "var(--mint)", marginBottom: 6 }}>{t.germany.koPathWinner}</h3>
        <div style={{ fontSize: 11, color: "var(--text-secondary)", lineHeight: 1.6 }}>
          • {t.germany.koPathWinnerR32}<br />
          • {t.germany.koPathWinnerR16}<br />
          • {t.germany.koPathWinnerQF}
        </div>
      </Card>
      <Card>
        <h3 style={{ fontSize: 13, fontWeight: 700, color: "var(--mint)", marginBottom: 6 }}>{t.germany.koPathSecond}</h3>
        <div style={{ fontSize: 11, color: "var(--text-secondary)", lineHeight: 1.6 }}>
          • {t.germany.koPathSecondR32}<br />
          • {t.germany.koPathSecondR16}
        </div>
      </Card>

      <SectionTitle>{t.germany.playerAggregatesTitle}</SectionTitle>
      <Card>
        {[
          { label: t.germany.statTop11, value: `${dtAgg.top11Markt} ${t.germany.statTop11Unit}` },
          { label: t.germany.statTop5Leagues, value: `${Math.round(dtAgg.pctTop5Ligen * 100)} %` },
          { label: t.germany.statUcl, value: `${dtAgg.uclSpieler}${t.germany.statUclMax}` },
          { label: t.germany.statXgAttack, value: `${dtAgg.xgAttacke.toFixed(1)} ${t.germany.statXgUnit}` },
          { label: t.germany.statXgaDefense, value: `${dtAgg.xgaDefense.toFixed(1)} ${t.germany.statXgUnit}` },
          { label: t.germany.statGkPsxg, value: `${(dtAgg.gkPsxg * 100).toFixed(0)} %` },
        ].map((r) => (
      ...
      <SectionTitle>{t.germany.profileTitle}</SectionTitle>
      <Card>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {profile.map(({ f, norm }) => (
            <div key={f.key}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                <span style={{ fontSize: 11, fontWeight: 600 }}>{f.icon} {factorLabel(f.key)}</span>
        ...
        <InfoBanner icon="★" tone="germany" style={{ marginTop: 12 }}>
          <strong>{t.germany.kaderMarktwert}</strong> ≈ {dt.markt} {t.germany.statTop11Unit}. {t.germany.keyPlayers}
        </InfoBanner>
      </Card>
    </section>
  );
